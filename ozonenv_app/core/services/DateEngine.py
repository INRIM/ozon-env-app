...
class DateEngine:
    ...
    def todaymax(self) -> datetime:
        return datetime.combine(date.today(), datetime.time.max)

    def eval_filter_delta(self, filter_data: list) -> int:
        if len(filter_data) > 1:
            ...
            if "n" in filter_data:
                ...
                num = 1
                if len(filter_data) > 1:
                    num = int(filter_data[2])
                num = num * sign
            return int(num)
        return 0

    def eval_year(self, filter_data: list) -> int:
        return self.curr_year + self.eval_filter_delta(filter_data)

    def eval_today(self, filter_data: list) -> str:
        days = self.eval_filter_delta(filter_data)
        min_max = time.min
        if "max" in filter_data:
            min_max = time.max
        return datetime.combine(
            (datetime.utcnow() + timedelta(days=days)).astimezone(self.tz),
            min_max,
        ).strftime(self.server_datetime_mask)

    def eval_date_filter(self, metakey: str):
        filter_base = metakey.replace("_date_", "")
        filter_data = filter_base.split("-")
        if "year" in filter_data:
            return self.eval_year(filter_data)
        if "today" in filter:
            return self.eval_today(filter_data)

    def today(self, days: int = 0) -> datetime:
        return (datetime.utcnow() + timedelta(days=days)).astimezone(self.tz)

    def year_range(
        self, year=0, datetime_o=datetime
    ) -> tuple[datetime, datetime]:
        if year == 0:
            year = date.today().year
        return datetime_o.min.replace(year=year), datetime_o.max.replace(
            year=year
        )

    def month_range(
        self, year=0, month=0, monthe=0, datetime_o=datetime
    ) -> tuple[datetime, datetime]:
        if year == 0:
            year = date.today().year
        if month == 0:
            month = date.today().month
        return datetime_o.min.replace(
            year=year, month=month
        ), datetime_o.max.replace(year=year, month=monthe)

    def get_date_from_server(self, date_to_parse: str) -> date:
        date_to_parse = datetime.strptime(
            date_to_parse, self.server_date_mask
        ).date()
        return date_to_parse

    def get_datetime_from_server(self, date_to_parse: str) -> datetime:
        date_to_parse = datetime.strptime(
    ...
    def gen_date_min_max_hours(
        self, min_day_delata_date_from=0, max_day_delata_date_to=1
    ):
        min = self.gen_date_delta_hours_from_today_masked(
            min_day_delata_date_from, self.server_date_mask
        )
    ...
    def gen_datetime_min_max_hours(
        ...
        min = self.gen_datetime_delta_hours_from_today_masked(
            min_hours_delata_date_from, self.server_datetime_mask
        )
        max = self.gen_datetime_delta_hours_from_today_masked(
            max_hours_delata_date_to, self.server_datetime_mask
        )
        return min, max
